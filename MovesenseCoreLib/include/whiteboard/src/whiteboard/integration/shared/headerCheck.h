...
#define WB_HEADER_CHECK_DEFINE(def)     \
    namespace _ ## def { \
        extern int WB_EVAL_AND_CONCATENATE(__header_check_failed_, def); \
        const int WB_ATTRIBUTE_UNUSED header_checker = WB_EVAL_AND_CONCATENATE(__header_check_failed_, def); \
    }
