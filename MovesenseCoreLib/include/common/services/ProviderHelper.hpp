...
#define RET_IF_ERROR(a)                                                                            \
    do {                                                                                           \
        ...
        if (!RETURN_OKC(_result_)) {                                                               \
            DEBUG_ASSERT(0); return _result_;                                                      \
        }                                                                                          \
    } while (0)
...
inline bool doAsyncSubscribe(
    whiteboard::ResourceClient* pClient, whiteboard::ResourceId resourceId,
    const whiteboard::ResourceClient::AsyncRequestOptions& rOptions =
    whiteboard::ResourceClient::AsyncRequestOptions::Empty)
{
...
#define RANGE_RETURN \
/* TRUE */    {returnResult(request, wb::HTTP_CODE_OK,  ResponseOptions::Empty, __tmp); return; } else \
/* FALSE */   {returnResult(request, wb::HTTP_CODE_OK,  ResponseOptions::Empty, wb::NoType::NoValue); return; }
#define ONGET_CASE_TYPE_RANGE(a, b, c, min, max)    case R_ID(a): do{ c __tmp = static_cast<c>(b); if(((b) >= (min)) && ((b) <= (max))) RANGE_RETURN } while(0)
#define ONGET_CASE_TYPE_RANGE_MAX(a, b, c, max)     case R_ID(a): do{ c __tmp = static_cast<c>(b); if((b) >= (min)) RANGE_RETURN } while(0)


#define ONSUB_CASE(a, b)            case R_ID(a): returnResult(request, wb::HTTP_CODE_OK,  ResponseOptions::Empty,  b); return
