...
#include "AutoClientGattConfig.h"

const char* const MainClient::LAUNCHABLE_NAME =
    "OfflineStorageGattClient";

// TODO: subscription status after finishing recording??
...
bool MainClient::startModule()
{
    ...
    // Start Logger.
    // TODO: search for status entries in memory before starting.
...
void MainClient::stopModule()
{
    ...
    // TODO: should trigger state update
                                           ...
                                           const wb::Value& rResultData)
{
    ...
    case WB_RES::LOCAL::MEM_LOGBOOK_BYID_LOGID_DATA::LID: {
        // TODO: parse and look for status entries
    ...
    case WB_RES::LOCAL::COMM_BLE_GATTSVC_SVCHANDLE_CHARHANDLE::LID: {
        ...
        if (charHandle == mCharAHandle)
        {
            if (!charValue.notifications.hasValue())
                return;
            if (charValue.notifications.getValue() && !this->mClientIsListeningToEcg) {
                this->asyncSubscribe(WB_RES::LOCAL::MEASUREMENTPROVIDER_ECG_DATASTREAM());
            } else if (!charValue.notifications.getValue() && this->mClientIsListeningToEcg) {
                this->asyncUnsubscribe(WB_RES::LOCAL::MEASUREMENTPROVIDER_ECG_DATASTREAM());
            }
        ...
        else if (charHandle == mCharBHandle)
        {
            if (!charValue.notifications.hasValue())
                return;
            if (charValue.notifications.getValue() && !this->mClientIsListeningToMov) {
                this->asyncSubscribe(WB_RES::LOCAL::MEASUREMENTPROVIDER_IMU9_DATASTREAM());
            } else if (!charValue.notifications.getValue() && this->mClientIsListeningToMov) {
                this->asyncUnsubscribe(WB_RES::LOCAL::MEASUREMENTPROVIDER_IMU9_DATASTREAM());
            }
            // TODO later MOV -> IMU
    ...
    const uint8_t* configFields)
{
        ...
        imuRecordingMode != mImuRecordingMode)
    {
        ...
        {
            // TODO: send state update to measprov
...
void MainClient::configureDataLoggerAll()
{
    ...
    WB_RES::DataEntry dlEntries[4];
    dlEntries[0].path = "/MeasurementProvider/ECG/LoggerStream";
    dlEntries[1].path = "/MeasurementProvider/IMU9/LoggerStream";
    dlEntries[2].path = "/MeasurementProvider/MeasurementStatus";
    dlConfig.dataEntries.dataEntry = wb::MakeArray(dlEntries, 3);
...
void MainClient::finishCurrentReadOperation()
{
    ...
    // TODO: finish and recover the most recent status

    // Start Data logger again.
    this->startDataLogger();
}
...
void MainClient::changeToLogging() {
    // TODO Send measProv logging instruction -> will pipe data only to logger
    // use mDataLoggingState
}

void MainClient::changeToStreaming() {
    // TODO Send measProv logging instruction -> will pipe data only to streamer
    // use mDataLoggingState
}
