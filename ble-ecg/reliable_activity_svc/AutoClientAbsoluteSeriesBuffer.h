...
template<class T>
class AutoClientAbsoluteSeriesBuffer
{
  public:
    /**
     * @brief Constructs a new Series Buffer.
     */
    AutoClientAbsoluteSeriesBuffer();
    /**
     * @brief Constructs a new Series Buffer.
     *
     ...
     */
    AutoClientAbsoluteSeriesBuffer(size_t bufferLength, size_t bufferCount);
    /**
     ...
     */
    void setTimestamp(absolute_timestamp_t t);
    /**
...
template<class T>
AutoClientAbsoluteSeriesBuffer<T>::AutoClientAbsoluteSeriesBuffer() :
    length(16),
    count(2),
    currentIndex(0),
    bufferedSamples(0)
...
template <class T>
AutoClientAbsoluteSeriesBuffer<T>::AutoClientAbsoluteSeriesBuffer(size_t bufferLength, size_t bufferCount) :
    length(bufferLength),
    count(bufferCount),
    currentIndex(0),
    bufferedSamples(0)
...
template <class T>
size_t AutoClientAbsoluteSeriesBuffer<T>::getSingleBufferSize()
{
    return sizeof(absolute_timestamp_t) + (this->length * sizeof(T));
}

template <class T>
uint8_t* AutoClientAbsoluteSeriesBuffer<T>::getBuffer(size_t i)
{
...
template <class T>
uint8_t* AutoClientAbsoluteSeriesBuffer<T>::getCurrentBuffer()
{
    return this->getBuffer(this->currentIndex);
}

template <class T>
void AutoClientAbsoluteSeriesBuffer<T>::resetCurrentBuffer()
{
    this->bufferedSamples = 0;
}

template <class T>
void AutoClientAbsoluteSeriesBuffer<T>::switchBuffer()
{
...
template <class T>
bool AutoClientAbsoluteSeriesBuffer<T>::canAddSample()
{
    return (this->bufferedSamples + 1) <= this->length;
}

template <class T>
bool AutoClientAbsoluteSeriesBuffer<T>::addSample(T sample)
{
    ...
    size_t offset = sizeof(absolute_timestamp_t) + (sizeof(T) * this->bufferedSamples);
...
template <class T>
void AutoClientAbsoluteSeriesBuffer<T>::setTimestamp(absolute_timestamp_t t)
{
    // Get a pointer to the start of the current buffer.
    uint8_t* currentBuffer = this->getCurrentBuffer();
    ...
    memcpy(currentBuffer, timestampBytes, sizeof(absolute_timestamp_t));
}

template <class T>
void AutoClientAbsoluteSeriesBuffer<T>::setLength(size_t len)
{
...
template <class T>
size_t AutoClientAbsoluteSeriesBuffer<T>::getLength()
{
    return this->length;
}
