...
const char* const DataProvider::LAUNCHABLE_NAME = "TwoWayMeasurementProvider";
...
static constexpr wb::LocalResourceId sProviderResources[] = {
    ...
    WB_RES::LOCAL::MEASUREMENTPROVIDER_ECG_LOGGERSTREAM::LID,
    WB_RES::LOCAL::MEASUREMENTPROVIDER_ECG_INTERVAL::LID,
    WB_RES::LOCAL::MEASUREMENTPROVIDER_IMU9_DATASTREAM::LID,
    WB_RES::LOCAL::MEASUREMENTPROVIDER_IMU9_LOGGERSTREAM::LID,
    WB_RES::LOCAL::MEASUREMENTPROVIDER_IMU9_INTERVAL::LID,
    WB_RES::LOCAL::MEASUREMENTPROVIDER_MEASUREMENTSTATUS::LID
};
DataProvider::DataProvider() :
    ...
    mainIsListeningECG(false),
    mainIsListeningIMU9(false)
{
    // Create buffers.
    this->ecgBuffer = new AutoClientAbsoluteSeriesBuffer<ecg_t>(DEFAULT_ECG_OBJECT_SIZE,
                                                      numberOfEcgBuffers);
    this->movBuffer = new AutoClientAbsoluteSeriesBuffer<mov_t>(DEFAULT_MOV_OBJECT_SIZE,
                                                      numberOfMovBuffers);
}
...
void DataProvider::deinitModule()
{
    unregisterProviderResources(sProviderResources);
    mModuleState = WB_RES::ModuleStateValues::UNINITIALIZED;
}

bool DataProvider::startModule()
{
...
void DataProvider::onPutRequest(const wb::Request& request,
...
void DataProvider::onSubscribe(const whiteboard::Request &rRequest, const whiteboard::ParameterList &rParameters) {
    switch (rRequest.getResourceId().localResourceId) {
        case WB_RES::LOCAL::MEASUREMENTPROVIDER_ECG_DATASTREAM::LID: {
            this->mainIsListeningECG = true;
            // Status Update.
            updateMeasState();
            return returnResult(rRequest, whiteboard::HTTP_CODE_OK);
        }
        case WB_RES::LOCAL::MEASUREMENTPROVIDER_IMU9_DATASTREAM::LID: {
            this->mainIsListeningIMU9 = true;
            // Status Update
            updateMeasState();
            return returnResult(rRequest, whiteboard::HTTP_CODE_OK);
        }
        default:
            return returnResult(rRequest, whiteboard::HTTP_CODE_NOT_FOUND);
    }
}

void DataProvider::onUnsubscribe(const whiteboard::Request &rRequest, const whiteboard::ParameterList &rParameters) {
    switch (rRequest.getResourceId().localResourceId) {
        case WB_RES::LOCAL::MEASUREMENTPROVIDER_ECG_DATASTREAM::LID: {
            this->mainIsListeningECG = false;
            // Status Update
            updateMeasState();
            return returnResult(rRequest, whiteboard::HTTP_CODE_OK);
        }
        case WB_RES::LOCAL::MEASUREMENTPROVIDER_IMU9_DATASTREAM::LID: {
            this->mainIsListeningIMU9 = false;
            // Status Update.
            updateMeasState();
        }
        default:
...
void DataProvider::onNotify(whiteboard::ResourceId resourceId,
...
void DataProvider::setEcgMeasurementInterval(uint8_t newEcgInterval)
{
...
void DataProvider::setImuMeasurementInterval(uint8_t newImuInterval)
{
    // Update interval and skip-counters for down-sampling.
...
ecg_t DataProvider::prepareEcgSample(int32 rawSample)
{
...
mov_t DataProvider::prepareImuSample(wb::FloatVector3D accSample,
...
acc_vec_t DataProvider::prepareAccSample(wb::FloatVector3D accVector)
{
...
gyr_vec_t DataProvider::prepareGyrSample(wb::FloatVector3D gyrVector)
{
...
mag_vec_t DataProvider::prepareMagSample(wb::FloatVector3D magVector)
{
...
void DataProvider::updateMeasState() {
    const WB_RES::MeasurementStatus status{this-> mainIsListeningECG ? this->mEcgMeasurementInterval : static_cast<uint8_t>(0),this->mainIsListeningIMU9 ? this->mImuMeasurementInterval : static_cast<uint8_t>(0)};
    updateResource(
        WB_RES::LOCAL::MEASUREMENTPROVIDER_MEASUREMENTSTATUS(),
        ResponseOptions::Empty, status);
}
