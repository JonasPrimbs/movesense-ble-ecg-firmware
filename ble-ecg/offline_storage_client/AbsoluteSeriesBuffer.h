...
class AbsoluteSeriesBuffer
{
  public:
     ...
     */
    AbsoluteSeriesBuffer(size_t bufferLength, size_t bufferCount);
    /**
...
template<class T>
AbsoluteSeriesBuffer<T>::AbsoluteSeriesBuffer() :
    length(16),
    count(2),
    currentIndex(0),
    bufferedSamples(0)
...
template <class T>
AbsoluteSeriesBuffer<T>::AbsoluteSeriesBuffer(size_t bufferLength, size_t bufferCount) :
    length(bufferLength),
    count(bufferCount),
    currentIndex(0),
    bufferedSamples(0)
...
template <class T>
size_t AbsoluteSeriesBuffer<T>::getSingleBufferSize()
{
    return sizeof(absolute_timestamp_t) + (this->length * sizeof(T));
}

template <class T>
uint8_t* AbsoluteSeriesBuffer<T>::getBuffer(size_t i)
{
...
template <class T>
uint8_t* AbsoluteSeriesBuffer<T>::getCurrentBuffer()
{
    return this->getBuffer(this->currentIndex);
}

template <class T>
void AbsoluteSeriesBuffer<T>::resetCurrentBuffer()
{
    this->bufferedSamples = 0;
}

template <class T>
void AbsoluteSeriesBuffer<T>::switchBuffer()
{
...
template <class T>
bool AbsoluteSeriesBuffer<T>::canAddSample()
{
    return (this->bufferedSamples + 1) <= this->length;
}

template <class T>
bool AbsoluteSeriesBuffer<T>::addSample(T sample)
{
...
template <class T>
void AbsoluteSeriesBuffer<T>::setTimestamp(absolute_timestamp_t t)
{
    // Get a pointer to the start of the current buffer.
    uint8_t* currentBuffer = this->getCurrentBuffer();
...
template <class T>
void AbsoluteSeriesBuffer<T>::setLength(size_t len)
{
...
template <class T>
size_t AbsoluteSeriesBuffer<T>::getLength()
{
    return this->length;
}
