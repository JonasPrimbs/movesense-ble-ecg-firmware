...
void CustomBleController::onBleInit()
{
    const char * id = GetDeviceIdentifier();
    std::size_t id_len = strlen(id);
    if (id_len >= BLE_PASSKEY_LEN)
        SetPassKey(id + id_len - BLE_PASSKEY_LEN);

//    SetBondingSettings(BONDING_POLICY_ENABLED, 0);
//    SetBondingSettings(BONDING_POLICY_DISABLED, 0);
//    SetBondingSettings(BONDING_POLICY_ONCE, 45);
    SetBondingSettings(BONDING_POLICY_SAME_MAC, 45);
}
